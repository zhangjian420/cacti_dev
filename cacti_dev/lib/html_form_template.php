...
function draw_nontemplated_fields_graph($graph_template_id, &$values_array, $field_name_format = '|field|', $header_title = '', $alternate_colors = true, $include_hidden_fields = true, $snmp_query_graph_id = 0,$title1="",$ifName=true) {
	global $struct_graph;
	...
	foreach ($struct_graph as $field_name => $field_array) {
		...
		}else if((!empty($snmp_query_graph_id)) && $field_name == "title"){
		    if(empty($title1)){ // 说明是多选的--将标题隐藏
				$form_array[$form_field_name]['method'] = 'hidden';
		    }
		    $form_array[$form_field_name]['value'] = empty($title1) ? 
		    ($ifName ? "|host_description| - 流量 - |query_ifName|" : "|host_description| - 流量 - |query_ifDescr|") : $title1;
		} /*---解决在创建图形的时候，可以直接填写图形的名称，所以注释掉了
...
function draw_pollerinterval_field_graph($local_graph_id,$header_title=""){
    $dtd = db_fetch_row_prepared("SELECT 
	dl.id AS local_data_id,dtd.`rrd_step`,dtd.`data_source_profile_id`
	FROM data_template_data dtd 
	LEFT JOIN data_local dl ON dtd.local_data_id = dl.id 
    LEFT JOIN graph_local gl ON dl.host_id = gl.host_id AND dl.snmp_query_id = gl.snmp_query_id AND dl.snmp_index = gl.snmp_index
WHERE gl.id = ? limit 1",array($local_graph_id));
    //cacti_log(json_encode($dtd));
    print "<div class='tableHeader'><div class='tableSubHeaderColumn'>数据源 [模板: 接口 - 流量]</div></div>\n";
    $form_array = array(
        'data_source_profile_id' => array(
            'method' => 'drop_sql',
            'friendly_name' => __('数据源配置文件'),
            'description' => __('选择数据源配置文件. 数据源配置文件控制结果数据源的采集周期,数据聚合和保留策略.'),
            'value' => $dtd["data_source_profile_id"],
            //'sql' => 'SELECT "0" AS id, "' . __('External') . '" AS name UNION SELECT id, name FROM data_source_profiles ORDER BY `default` desc'
            'sql' => 'SELECT id, name FROM data_source_profiles ORDER BY `default` desc'
        )
    );
    draw_edit_form(
        array(
