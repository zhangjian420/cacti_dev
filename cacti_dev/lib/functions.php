...
function filter_value($value, $filter, $href = '',$target="_self") {
	static $charset;
	...
	if ($href != '') {
		$value = '<a class="linkEditMain" target="'.$target.'" href="' . htmlspecialchars($href, ENT_QUOTES, $charset, false) . '">' . $value  . '</a>';
	}
...
function read_user_setting($config_name, $default = false, $force = false, $user = 0) {
	...
	if ($user == 0 && isset($_SESSION['sess_user_id'])) {
		...
		$db_setting = false;
		if (db_table_exists('settings_user')) {
			$db_setting1 = db_fetch_row_prepared('SELECT value
				FROM settings_user
				WHERE name = ?
				AND user_id = ?',
				array($config_name, $effective_uid));
		}
		if (db_table_exists('settings_user_group')) {
		    $db_setting2 = db_fetch_row_prepared('SELECT value
				FROM settings_user_group
				WHERE name = ?
				AND group_id = (select group_id from user_auth_group_members where user_id = ?)',
		        array($config_name, $effective_uid));
		}
		$db_setting = array_merge($db_setting1,$db_setting2);
...
function is_raise_message() {
    if (isset($_SESSION['sess_messages']) && sizeof($_SESSION['sess_messages'])) {
        return true;
    } else {
        return false;
    }
...
function display_custom_error_message($message) {
	raise_message('custom_error', "<span style=\"color:red\">".$message."</span>");
}
...
function cacti_logdb($string){
    $message = clean_up_lines($string) . PHP_EOL;
    db_execute_prepared("insert into user_op_log(user_id,user_name,ip,insert_time,content,status) 
        values(?,?,?,?,?,?)",
        array(
            empty($_SESSION['sess_user_id']) ? 0 : $_SESSION['sess_user_id'],
            empty($_SESSION['sess_user_username']) ? "" : $_SESSION['sess_user_username'],
            get_client_addr(''), date('Y-m-d H:i:s', time()),$message,1));
}
...
function cacti_log($string, $output = false, $environ = 'CMDPHP', $level = '') {
	...
	if(!empty($_SESSION['sess_user_username']) && !empty(get_client_addr(''))){
	    $user_and_ip = "[".$_SESSION['sess_user_username']."] - " .get_client_addr('');
	}else{
	    $user_and_ip = "";
	}
	/* format the message */
	if ($environ == 'POLLER') {
	    $prefix = "$date - " . $environ . $user_and_ip . ': Poller[' . $config['poller_id'] . '] ';
	} else {
	    $prefix = "$date - " . $environ . $user_and_ip . ' ';
	}
...
function draw_login_status($using_guest_account = false) {
	...
	if (isset($_SESSION['sess_user_id']) && $_SESSION['sess_user_id'] == $guest_account) {
		...
		print ("<li><a href='" . $config['url_path'] . "auth_profile.php?action=edit'>" . __('Edit Profile') . "</a></li>");
...
function mailer($from, $to, $cc, $bcc, $replyto, $subject, $body, $body_text = '', $attachments = '', $headers = '', $html = true) {
	...
	if ($how == 0) {
		...
		if (read_config_option('settings_smtp_username') != '') {
			$mail->SMTPAuth = true;
			$settings_smtp_username = read_config_option('settings_smtp_username');
			$usernames = explode(";", $settings_smtp_username);
			if(sizeof($usernames) == 1){
			    $mail->Username = $usernames[0];
			}else{
			    $key = array_rand($usernames,1);
			    $mail->Username = $usernames[$key];
			}
			if(is_array($from)){
			    $from[0] = $mail->Username;
			}else{
			    $from = $mail->Username;
			}
...
function raise_ajax_permission_denied() {
	if (is_page_ajax()) {
		header('HTTP/1.1 401 ' . __('权限拒绝'));
		print __('没有访问权限') . '  ' . __('如果有疑问，请联系管理员');
		exit;
	}
}

/**
 * 下载模板，文件的目录必须是 /tpl目录下，并且必须是zip文件
 */
function download_tpl($filename){
    global $config;
    $file = $config['base_path'] . "/tpl/$filename";
    if (file_exists($file)) {
        $fh = fopen($file, "rb" );
 ...
 * @return number
 */
function get_data_source_by_id($local_data_id){
    global $data_source_values;
    if(empty($local_data_id)){
        return 0;
    }
    if(empty($data_source_values)){
        $data_locals = db_fetch_assoc("select * from data_local");
        foreach ($data_locals as $data_local){
            $data_source_values[$data_local["id"]] = $data_local;
        }
    }
    return $data_source_values[$local_data_id];
}