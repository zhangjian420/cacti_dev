...
function rrdtool_function_update($update_cache_array, $rrdtool_pipe = '') {
	...
	$rrds_processed = 0;
	// {"\/var\/www\/html\/cacti\/rra\/162\/4071.rrd":
	//{"local_data_id":"4071","times":{"1576253161":{"traffic_in":"5212601458","traffic_out":"52603375338"}}}}
	foreach ($update_cache_array as $rrd_path => $rrd_fields) {
		...
		if (is_array($rrd_fields['times']) && cacti_sizeof($rrd_fields['times'])) {
			...
			$data_source = get_data_source_by_id($rrd_fields['local_data_id']);
			$prepare_value = 1050;
			if(!empty($data_source) && !empty($data_source["prepare_value"])){
			    $prepare_value = $data_source["prepare_value"];
			}
			$prepare_op = 1;
			if(!empty($data_source) && !empty($data_source["prepare_op"])){
			    $prepare_op = $data_source["prepare_op"];
			}
			$upper_limit = 0;
			if (!empty($data_source) && !empty($data_source["upper_limit"])) {
			    $upper_limit = $data_source["upper_limit"];
			}
			...
			foreach ($rrd_fields['times'] as $update_time => $field_array) {
				...
				foreach ($field_array as $field_name => $value) {
					...
					// 如果获取的是出入口流量，由于此值偏大，所以进行了特殊处理
					if(($field_name == "traffic_in" || $field_name == "traffic_out") && $value != 'U'){
					    //cacti_log("处理前 = " . $field_name . " = " . $value . " prepare_op = " . $prepare_op . "  prepare_value = " . $prepare_value . "  upper_limit = " . $upper_limit);
					    if($prepare_op == 2){
    					    $value = number_format(round($value - $prepare_value), 0, '', '');
					    }elseif ($prepare_op == 3){
    					    $value = number_format(round($value + $prepare_value), 0, '', '');
					    }elseif ($prepare_op == 4){
					        $value = number_format(round($value * $prepare_value), 0, '', '');
					    }else{
					        $value = number_format(round($value * 1024 / $prepare_value), 0, '', '');
					    }
					    //cacti_log("处理后 = " . $field_name . " = " . $value);
					}
...
// $resolution 这个参数是新加的，想测试下看看是否可以根据图形id动态调整粒度来画图
function rrdtool_function_graph($local_graph_id, $rra_id, $graph_data_array, $rrdtool_pipe = '', &$xport_meta = array(), $user = 0,$resolution=60) {
	...
	$ds_step = $resolution;
	//cacti_log("rra_id = " . $rra_id. " local_data_ids = " .json_encode($local_data_ids)." graph_data_array = " . json_encode($graph_data_array) . " ds_step = " . $ds_step);
	...
	if (empty($rra_id)) {
		...
		$rra = db_fetch_row_prepared('SELECT
			...
			WHERE dspr.data_source_profile_id = ? limit 1',
			array($rra_id)
		);
	...
	/* +++++++++++++++++++++++ GRAPH OPTIONS +++++++++++++++++++++++ */
	//cacti_log("graph_start = " .$graph_start . " graph_end = " . $graph_end . " graph_data_array = ".json_encode($graph_data_array) . " ds_step = " . $ds_step);
	...
	//cacti_log("graph_opts = " .$graph_opts);
	/* +++++++++++++++++++++++ LEGEND: MAGIC +++++++++++++++++++++++ */
...
function rrdtool_function_xport($local_graph_id, $rra_id, $xport_data_array, &$xport_meta,$resolution=60) {
    return rrdtool_function_graph($local_graph_id, $rra_id, $xport_data_array, '', $xport_meta,0,$resolution);
}
