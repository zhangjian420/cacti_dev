...
function html_start_box($title, $width, $div, $cell_padding, $align, $add_text, $add_label = false) {
	...
	if ($title != '') {
		...
		if ($add_text != '' && !is_array($add_text)) {
			print "<span class='cactiFilterAdd' title='$add_label'><a class='linkOverDark' title='$add_label' href='" . html_escape($add_text) . "'><i class='fa fa-plus'></i></a></span>";
		} else {
			if (is_array($add_text)) {
				if (cacti_sizeof($add_text)) {
					foreach($add_text as $icon) {
						...
						print "<span class='cactiFilterAdd' title='$title'><a" . (isset($icon['id']) ? " id='" . $icon['id'] . "'":"") . " class='$classo' title='$title' href='$href'><i class='$classi'></i></a></span>";
					}
				}
			} else {
				print "<span> </span>";
...
function html_show_tabs_left() {
	...
	if (get_selected_theme() == 'classic') {
		...
		if ($realm_allowed[21] || $realm_allowed[22]) {
		    if(isset($_SESSION['sess_reports_status']) && $_SESSION['sess_reports_status'] == "off"){
				// Don't show reports tabe if lic off
		    }else if ($config['poller_id'] > 1) {
		...
		if ($realm_allowed[18]) {
			if (substr_count($_SERVER['REQUEST_URI'], 'clog')) {
				print '<a id="tab-logs" href="' . $config['url_path'] . ($realm_allowed[18] ? 'clog.php':'clog_user.php') . '"><img src="' . $config['url_path'] . 'images/tab_clog_down.png" alt="调试输出"></a>';
			} else {
				print '<a id="tab-logs" href="' . $config['url_path'] . ($realm_allowed[18] ? 'clog.php':'clog_user.php') . '"><img src="' . $config['url_path'] . 'images/tab_clog.png" alt="调试输出"></a>';
			}
		}
		...
		foreach($tabs_left as $tab) {
			...
			print "<li><a id='$id' class='lefttab" . (isset($tab['selected']) ? ' selected':'') 
			. "' href='" . html_escape($tab['url']) . "'><span class='fa glyph_$id'></span><span class='text_$id'>" 
				    . html_escape($tab['title']) . "</span></a><a id='menu-$id' class='maintabs-submenu' href='#'><i class='fa fa-angle-down'></i></a></li>\n";
		}
...
function html_common_header($title, $selectedTheme = '') {
	...
	print get_md5_include_css('include/themes/' . $selectedTheme .'/jquery.searchableSelect.css');
	...
	print get_md5_include_js('include/js/jquery.searchableSelect.js');
