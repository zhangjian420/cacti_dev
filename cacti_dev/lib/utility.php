...
/**
 * 在新建图形的时候，将图形的上限、下限和流量数据源流入和流出 最大值、最小值 同步
 */
function syn_traffic_save_datasrc_rrd($local_graph_id,$local_data_id){
    $limit = db_fetch_row_prepared('SELECT upper_limit,lower_limit
					FROM graph_templates_graph
					WHERE local_graph_id = ?',
        array($local_graph_id));
    //cacti_log("获取  local_graph_id = ".$local_graph_id." 中设置的 upper_limit = ".$limit["upper_limit"]."，lower_limit = ".$limit["lower_limit"],true);
    
    db_execute_prepared('update data_template_rrd set rrd_maximum = ?,rrd_minimum = ? where
						local_data_id = ?',
        array($limit["upper_limit"],$limit["lower_limit"], $local_data_id));
    //cacti_log("成功更新数据源id = ".$local_data_id." 出入口流量最大值 ",true);
}

/**
 * 在修改图形的时候，需要检查图形上线是否变化，如果变化，需要同步修改数据源中流入和流出的上线值，并且执行命令，同步rrd文件，否则
 * rrd文件一直存在值不同步的问题，执行脚本即可。
 */
function syn_traffic_update_datasrc_rrd($local_graph_id,$upper_limit,$data_source_profile_id){
    $local_data_id = db_fetch_cell_prepared("select dl.id from data_local dl left join graph_local gl 
            on gl.host_id = dl.host_id and gl.snmp_query_id = dl.snmp_query_id and gl.snmp_index = dl.snmp_index 
            where gl.id = ? limit 1"
        ,array($local_graph_id));
    
    $data_template_rrd = db_fetch_row_prepared("select * from data_template_rrd where local_data_id = ? limit 1",array($local_data_id));
    if(!empty($data_template_rrd) && $data_template_rrd["rrd_maximum"] != $upper_limit){ // 说明在页面中修改了上线值
        // 1、更新上限值
        db_execute_prepared('update data_template_rrd set rrd_maximum = ? where local_data_id = ?',
            array($upper_limit, $local_data_id));
        // 2、修改rrd文件的上限值
        $rrdtool_path = read_config_option('path_rrdtool');
        $data_source_path = get_data_source_path($local_data_id,true);
        $cmd1 = "sudo ".$rrdtool_path . ' tune ' . $data_source_path . " --maximum traffic_in:".$upper_limit;
        $cmd2 = "sudo ".$rrdtool_path . ' tune ' . $data_source_path . " --maximum traffic_out:".$upper_limit;
        shell_exec($cmd1);
        shell_exec($cmd2);
//         cacti_log("cmd1 = " . $cmd1);
//         cacti_log("cmd2 = " . $cmd2);
//         cacti_log("上线修改完成");
    ...
    syn_traffic_update_datasrc_rrd_by_data_id($local_data_id,$data_source_profile_id,"");
}

/**
 * 根据数据源id同步step值
 * @param $local_data_id
 * @param $data_source_profile_id
 */
function syn_traffic_update_datasrc_rrd_by_data_id($local_data_id,$data_source_profile_id,$rrd_step){
    $data_template_data = db_fetch_row_prepared("select * from data_template_data where local_data_id = ? limit 1",array($local_data_id));
    if (!empty($data_template_data)) { // 说明用户修改了监控时间
        // 更新数据源中的step，更新后会造成数据源有问题，这时候需要重新创建数据源
        $step = db_fetch_cell_prepared("select step from data_source_profiles where id = ?",array($data_source_profile_id));
        if($data_template_data["data_source_profile_id"] != $data_source_profile_id || (!empty($rrd_step) && $step != $rrd_step)){
            //cacti_log("step = " . $step . " data_source_profile_id = " . $data_source_profile_id);
            db_execute_prepared('update data_template_data set rrd_step = ?,data_source_profile_id = ?
        			where local_data_id = ?',array($step,$data_source_profile_id,$local_data_id));
            // 1、删除之前的的rrd文件
            $data_source_path = get_data_source_path($local_data_id,true);
            $cmd1 = "sudo rm -rf " . $data_source_path;
            shell_exec($cmd1);
            //         cacti_log("cmd1 = " . $cmd1);
            //         cacti_log("删除rrd文件完成");
            // 2、重新创建
            rrdtool_function_create($local_data_id, "-600", false);
            //         cacti_log("重建rrd文件完成");
        }
    }
}