		...
		foreach ($fields_array as $field_name => $field_array) {
			if ($field_array['method'] == 'hidden') {
				...
				if (isset($config_array['force_row_color'])) {
				    print "<div id='row_$field_name' class='formRow even-alternate $row_class ".(isset($field_array["rowClass"]) ? $field_array["rowClass"] : '') ."'>";
				} else {
				    print "<div id='row_$field_name' class='formRow $row_class ".(isset($field_array["rowClass"]) ? $field_array["rowClass"] : '')."'>";
					if ($row_class == 'even') {
						$row_class = 'odd';
					} else {
						$row_class = 'even';
...
function form_save_buttons($buttons,$include_save = true) {
	?>
	<table style='width:100%;text-align:center;'>
		<tr>
			<td class='saveRow'>
