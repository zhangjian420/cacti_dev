...
function html_graph_custom_data($host_id, $host_template_id, $snmp_query_id, $form_type, $form_id1, $form_array2) {
	...
	$title1 = "";$ifName=true;
	if(!empty($num_graphs)){ // 只是创建了一个图形的时候，标题要加入端口
	    if($num_graphs == 1){
	        $pam = array_keys($form_array3);
	        array_push($pam, $host_id,$snmp_query_id);
	    }else {
	        $x = array_keys($form_array3)[0]; // 为了看ifName是否有值
	        $pam = array();
	        $pam[0] = $x;
	        array_push($pam, $host_id,$snmp_query_id);
	    }
	    
	    $hsc = db_fetch_assoc_prepared("select field_name,field_value from host_snmp_cache where 
            md5(snmp_index) = ? and host_id = ? and snmp_query_id = ? and field_name in ('ifName','ifDescr')",$pam);
	    foreach ($hsc as $v){
	        if($v["field_name"] == "ifName" && !empty($v["field_value"])){
	            $title1 = "|host_description| - 流量 - " . $v["field_value"];
	            if ($num_graphs != 1) {
	                $ifName = true;
	            }
	            break;
	        }
	        if($v["field_name"] == "ifDescr" && !empty($v["field_value"])){
	            $title1 = "|host_description| - 流量 - " . $v["field_value"];
	            if ($num_graphs != 1) {
	                $ifName = false;
	            }
	    ...
	    if ($num_graphs != 1) { // 多选创建图形的时候，标题需要清空，使用根据判断选择使用ifName或者ifDescr
	        $title1 = "";
	    }
	}
	array_push($num_output_fields, draw_nontemplated_fields_graph($graph_template_id, $graph_template, "g_$snmp_query_id" . '_' . $graph_template_id . '_|field|', __('Graph [Template: %s]', html_escape($graph_template['graph_template_name'])), true, false, (isset($snmp_query_graph_id) ? $snmp_query_graph_id : 0),$title1,$ifName));
