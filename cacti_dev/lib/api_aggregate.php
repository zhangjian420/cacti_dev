...
// 	cacti_log(__FUNCTION__ . ' 被调用，_old_graph_id=' . $_old_graph_id 
// 	    . '， _graph_template_id=' . $_graph_template_id . '， _new_graph_id =' . $_new_graph_id 
// 	    . '，_graph_item_sequence=' . $_graph_item_sequence . '， _selected_graph_index =' . $_selected_graph_index 
// 	    . '，_graph_type= ' . $_graph_type.",_total = ".$_total.",_gprint_prefix=".$_gprint_prefix.",_total_type = "
// 	    .$_total_type.",_graph_item_types=".json_encode($_graph_item_types)
// 	    .",_skip=".json_encode($_skip).",_totali=".json_encode($_totali));
	...
	if (!empty($_old_graph_id)) {
		...
		$graph_local = db_fetch_row_prepared('SELECT host_id, graph_template_id,
			...
			WHERE id = ?',
			array($_old_graph_id));
// 		cacti_log("_old_graph_id = " .$_old_graph_id ."不为空，根据它查询到图形模板item = ".json_encode($graph_items) 
// 		    . ",查询到图形 = " .json_encode($graph_local));
	} else {
		$graph_items = db_fetch_assoc_prepared('SELECT *
			FROM graph_templates_item
			WHERE local_graph_id = ?
...
// 		cacti_log("_old_graph_id = " .$_old_graph_id ."，graph_template_id = $_graph_template_id 为空，根据它查询到图形模板item = ".json_encode($graph_items)
// 		    . ",查询到图形 = " .json_encode($graph_local));
		$graph_local = array();
	}
...
// 		    cacti_log("正在循环的graph_item = ".json_encode($graph_item));
...
// 						    cacti_log("位置3 = ".json_encode($graph_item));
							continue;
						}
					}
				}
...
// 			    cacti_log("进入total的判断");
				...
				$graph_item['graph_type_id'] == GRAPH_ITEM_TYPE_STACK) {
				    $graph_item['text_format'] = ($_gprint_prefix == "一共" ?"" : $_gprint_prefix) . $graph_item['text_format'];
			}
...
// 			    cacti_log("进入颜色模板  = " .$_color_templates[$i]);
...
// 			    cacti_log("进入cdef");
			...
			if ($_total_type != '') {
// 			    cacti_log("进入_total_type = " .$_total_type.",prepend = "
// 			        .$prepend.",prepend_cnt=".$prepend_cnt.",text_format=".$save['text_format']);
				if ($prepend) {
...
// 					if(!empty($_gprint_prefix) && $_gprint_prefix != $save['text_format']){ // 如果给了前缀，并且前缀和原始名字不同，添加上前缀
// 					    $save['text_format'] = $_gprint_prefix . $save['text_format'];
// 					}
...
// 					cacti_log("经过第二次处理后的text_format = " .$save['text_format']);
...
// 			cacti_log("需要保存的图形模板item = ".json_encode($save));
...
function push_out_aggregates($aggregate_template_id, $local_graph_id = 0) {
    cacti_count("开始push_out 聚合图形 aggregate_template_id = $aggregate_template_id ，local_graph_id = ".$local_graph_id);
	$attribs                    = array();
...
// 	    cacti_log("进入if");
		$id = db_fetch_cell_prepared('SELECT id
			...
			WHERE local_graph_id = ?',
			array($local_graph_id));
// 		cacti_log("根据聚合图形中local_graph_id = $local_graph_id 获取聚合图形id= " .$id);

		$attribs['graph_title'] = db_fetch_cell_prepared('SELECT title_format
			FROM aggregate_graphs
			WHERE id = ?',
			array($id));
// 		cacti_log("根据聚合图形id = $id 获取图形的标题 = " .$attribs['graph_title']);
...
// 		cacti_log("需要排除的序列集合 = " . json_encode($attribs['skipped_items']));
...
// 		cacti_log("需要计算total的序列集合 = " . json_encode($attribs['total_items']));
...
// 	cacti_log("最后得到attribs = " .json_encode($attribs));
	...
	if ($local_graph_id > 0) {
		...
		$graphs = db_fetch_assoc_prepared('SELECT local_graph_id
			...
			WHERE aggregate_template_id = ?',
			array($aggregate_template_id));
// 		cacti_log("根据聚合图形模板id = $aggregate_template_id 获取到local_graph 集合 = " . json_encode($graphs));
...
// 	cacti_log("最后得到local_graph 集合 = " . json_decode($aggregate_graphs));
	...
	if (cacti_sizeof($aggregate_graphs)) {
		foreach($aggregate_graphs as $ag) {
			...
			$graphs        = db_fetch_assoc_prepared('SELECT DISTINCT agi.local_graph_id
				...
				array($ag));
// 			cacti_log("根据聚合图形中local_graph_id = $ag 去重复获取聚合图形item 中的 local_graph_id = " .json_encode($graphs));
			...
			if ($ag > 0) {
				db_execute_prepared('DELETE FROM graph_templates_item
					WHERE local_graph_id = ?',
					array($ag));
// 			    cacti_log("根据local_graph_id = $ag 删除graph_templates_item 中中的所有结果");
			}
...
//     cacti_log("聚合图形创建和更新 local_graph_id = " .$local_graph_id .",attribs = ".json_encode($attribs));
	global $config;
...
// 			cacti_log("创建一个新的的图形 最后得到的local_graph_id= " .$local_graph_id);
			$save1['id']    = '';
			$new_aggregate  = true;
		} else {
...
// 			cacti_log("更新一个新的的图形 最后得到的local_graph_id= " .$local_graph_id);
			$save1['id'] = db_fetch_cell_prepared('SELECT id
				FROM aggregate_graphs
				WHERE local_graph_id = ?',
				array($local_graph_id));
// 			cacti_log("根据local_graph_id = $local_graph_id 查询到需要更新的聚合图形表id = " .$save1['id']);
			$new_aggregate  = false;
		}
...
// 		cacti_log("保存聚合图形aggregate_graphs表成功，得到的id = $aggregate_graph_id");
		...
		if ($local_graph_id > 0) {
			db_execute_prepared('DELETE FROM graph_templates_item
				WHERE local_graph_id = ?',
				array($local_graph_id));
// 			cacti_log("根据local_graph_id = $local_graph_id 删除graph_templates_item表中的所有item");
		}
...
// 		    cacti_log("第一次保存1 = skipped_items = " .json_encode($skipped_items) . ",total_items = " .  json_encode($total_items));
			$next_item_sequence = aggregate_graphs_insert_graph_items(
				...
				'',
				$member_graphs);
// 			cacti_log("根据 local_graph_id = $graph_id 插入新的图形item，得到的结果 = " .$next_item_sequence);
...
//             cacti_log("通过replace into更新聚合图形item，参数 aggregate_graph_id = $aggregate_graph_id , local_graph_id = $graph_id ， j = $j");
...
// 				cacti_log("第二次保存1 = skipped_items = " .json_encode($skipped_items) . ",total_items = " . json_encode($total_items));
				for ($k=1; $k<=$item_no; $k++) {
...
// 					cacti_log("total_items k = $k , v = " . $total_items[$k]);
...
// 				cacti_log("第二次保存2 = skipped_items = " .json_encode($skipped_items) . ",total_items = " . json_encode($total_items));
