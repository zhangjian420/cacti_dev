			...
			if (!empty($user_info)) {
				if (isset($parts[1])) {
					...
					if (empty($found)) {
						...
						cacti_log("用户 [" . $user_info['username'] . "] - ".get_client_addr('')." 通过Cookie登录系统", false, 'AUTH');
...
function get_allowed_tree_level($tree_id, $parent_id, $editing = false, $user = 0) {
    $query_sql = 'SELECT gti.id, gti.title, gti.host_id,
		...
		ON s.id = gti.site_id
        left join host as h1
        on h1.id = gti.graph_host_id
        LEFT JOIN user_auth_perms b1
        ON b1.item_id = h1.id AND b1.type = 3 and b1.user_id = '.$_SESSION['sess_user_id'].'
        left join user_auth_perms b2
        on b2.item_id = gti.local_graph_id and b2.type = 1 and b2.user_id = '.$_SESSION['sess_user_id'].'
		WHERE gti.graph_tree_id = ?
		AND gti.parent = ?
        and b1.item_id is null
        and b2.item_id is null
		ORDER BY gti.position ASC';
    $items = db_fetch_assoc_prepared($query_sql,
...
function get_allowed_tree_content($tree_id, $parent = 0, $sql_where = '', $order_by = '', $limit = '', &$total_rows = 0, $user = 0) {
	...
	if ($tree_id > 0) {
		...
		$heirarchy = db_fetch_assoc("SELECT gt.id AS tree_id, '0' AS id, gt.name AS title, '0' AS host_id, '0' AS site_id,
            ...
            LEFT JOIN user_auth_group_perms c on c.item_id = gt.id and c.type = 2 
            and c.group_id = (select group_id from user_auth_group_members where user_id =". $_SESSION['sess_user_id'] . ") 
			WHERE enabled='on' and c.item_id is null
	...
	if ($auth_method != 0) {
		...
		if ($total_rows != -2) {
		    $query_sql = "SELECT h1.*
				...
				$order_by
				$limit";
			//cacti_log("host1 = " . $query_sql);
			$host_list = db_fetch_assoc($query_sql);
		}
