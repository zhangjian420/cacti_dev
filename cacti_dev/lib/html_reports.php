...
function reports_form_save() {
	...
	if (isset_request_var('save_component_report')) {
		...
		cacti_logdb((empty(get_nfilter_request_var('id'))? "添加报告" : "修改报告").$save['name']);
...
function reports_send($id) {
	...
	if (!cacti_sizeof($report)) {
		/* set error condition */
	} elseif ($report['user_id'] == $_SESSION['sess_user_id'] || $_SESSION['sess_user_id'] = 1) {
...
function reports_item_edit() {
	...
	$report_item = array();
	$report_item['item_type'] = get_request_var('item_type',REPORTS_ITEM_GRAPH);
	...
	if (isset($reports_item['item_type']) && $reports_item['item_type'] == REPORTS_ITEM_GRAPH) {
		...
		if (empty($reports_item['timespan'])) {
		    $reports_item['timespan'] = 7;
		}
...
function display_reports_items($report_id) {
	...
	html_header(
		array(
			...
			array('display' => __('类型'),         'align' => 'left'),
...
function reports() {
	...
	/* account for permissions ----10.7 通过权限授予*/
	/*if (is_reports_admin()) {
	...
	$sql_join = "LEFT JOIN user_auth_group_perms b on b.item_id = reports.id and b.type = 102 
	...
	if (is_reports_admin()) {
		$display_text = array(
			...
			'attachment_type' => array('display' => __('图像附加类型'),        'align' => 'left', 'sort' => 'ASC'),
	...
	if (cacti_sizeof($reports_list)) {
		...
		foreach ($reports_list as $report) {
			...
			form_selectable_ecell(get_username($report['user_id']), $report['id']);
