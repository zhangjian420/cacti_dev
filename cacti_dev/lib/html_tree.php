			...
			.on('select_node.jstree', function(e, data) {
				if (data.node.id) {
					...
					if (typeof href !== 'undefined') {
						...
						href = href + '&hyper=true&button_clear=1&clear_timespan=1&date1=1';
...
function html_validate_tree_vars() {
	...
	$filters = array(
		...
		'columns' => array(
			'filter' => FILTER_VALIDATE_INT,
			'default' => read_user_setting('num_columns_tree', '1')
			),
		'page' => array(
			'filter' => FILTER_VALIDATE_INT,
			'default' => '1'
...
function grow_right_pane_tree($tree_id, $leaf_id, $host_group_data) {
	...
	///---------------这里过滤
	$graph_list = filterPermsGraphs($graph_list);
...
function filterPermsGraphs($graphs){
    $local_graph_ids = "";
    foreach ($graphs as $graph){
        $local_graph_ids .= $graph["local_graph_id"].",";
    }
    $local_graph_ids = substr($local_graph_ids,0,strlen($local_graph_ids)-1); 
    if(!empty($local_graph_ids)){
        $query_sql = "select id from graph_local gl
        LEFT JOIN user_auth_perms b1
        ON b1.item_id = gl.host_id AND b1.type = 3 and b1.user_id = ".$_SESSION['sess_user_id']."
        ...
        and b2.item_id is null and gl.id in (".$local_graph_ids.")";
        $graph_locals = db_fetch_assoc($query_sql); // 查询可见图形
        $graph_locals_ids = array_column($graph_locals,"id");
        
        $new_graphs = array();
        foreach ($graphs as $graph){
            if(in_array($graph["local_graph_id"], $graph_locals_ids)){
                array_push($new_graphs, $graph);
            }
        }
        return $new_graphs;
    }
