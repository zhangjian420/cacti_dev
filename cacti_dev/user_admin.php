...
function update_policies() {
	...
	$set .= isset_request_var('policy_topos') ? ($set != '' ? ',':'') . 'policy_topos=' . get_nfilter_request_var('policy_topos'):'';
	$set .= isset_request_var('policy_reps') ? ($set != '' ? ',':'') . 'policy_reps=' . get_nfilter_request_var('policy_reps'):'';
	$set .= isset_request_var('policy_forms') ? ($set != '' ? ',':'') . 'policy_forms=' . get_nfilter_request_var('policy_forms'):'';
...
function form_actions() {
	...
	if (isset_request_var('associate_host')) {
		...
		exit;
	}elseif (isset_request_var('associate_topo')) {
	    foreach ($_POST as $var => $val) {
	        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
	            /* ================= input validation ================= */
	            input_validate_input_number($matches[1]);
	    ...
	    header('Location: user_admin.php?action=user_edit&header=false&tab=permstopo&id=' . get_nfilter_request_var('id'));
	    exit;
	}elseif (isset_request_var('associate_rep')) {
	    foreach ($_POST as $var => $val) {
	        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
	            /* ================= input validation ================= */
	            input_validate_input_number($matches[1]);
	    ...
	    header('Location: user_admin.php?action=user_edit&header=false&tab=permsrep&id=' . get_nfilter_request_var('id'));
	    exit;
	}elseif (isset_request_var('associate_form')) {
	    foreach ($_POST as $var => $val) {
	        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
	            /* ================= input validation ================= */
	            input_validate_input_number($matches[1]);
	    ...
	    header('Location: user_admin.php?action=user_edit&header=false&tab=permsform&id=' . get_nfilter_request_var('id'));
	    exit;
...
function form_save() {
	...
	if ((isset_request_var('save_component_graph_perms')) && (!is_error_message())) {
		...
		get_filter_request_var('policy_topos');
		get_filter_request_var('policy_reps');
		get_filter_request_var('policy_forms');
		...
		if (isset($_SESSION['sess_user_num_lic']) && ($user_num) >= $_SESSION['sess_user_num_lic'] && empty(get_request_var("id"))) {
		    raise_message(1002,"用户最多创建".$_SESSION['sess_user_num_lic']."个，若增加用户请重新授权！",MESSAGE_LEVEL_ERROR);
		}
		...
		$save["white_ips"]            = get_nfilter_request_var("white_ips");
		$save["user_auth_group_id"]   = get_nfilter_request_var("user_auth_group_id");
		if ($save['locked'] == '') {
			$save['failed_attempts'] = 0;
		}

		if (!is_error_message() && !is_raise_message()) {
			...
			db_execute("insert into user_auth_group_members values(".$save["user_auth_group_id"].",".$user_id.")");
		...
		db_execute_prepared('DELETE FROM user_auth_realm
			WHERE user_id = ?',
			array(get_nfilter_request_var('id')));
		db_execute_prepared('update user_auth set is_realm_perms_saved = 1 
			WHERE id = ?',
		    array(get_nfilter_request_var('id')));
		foreach ($_POST as $var => $val) {
		...
		db_execute_prepared('UPDATE user_auth
			...
			array(
				...
				get_nfilter_request_var('policy_topos'),
				get_nfilter_request_var('policy_reps'),
				get_nfilter_request_var('policy_forms'),
...
function graph_perms_edit($tab, $header_label) {
	...
	if (!isempty_request_var('id')) {
		$policy = db_fetch_row_prepared('SELECT policy_graphs, policy_trees,policy_topos,policy_reps,policy_forms, policy_hosts, policy_graph_templates
			FROM user_auth
	...
	case 'permsg': // 图形
		...
		$sql111 = "SELECT gtg.local_graph_id, h.description,
			...
			$limit";
		//cacti_log("sql = " . $sql111);
		$graphs = db_fetch_assoc($sql111);
		...
		if (cacti_sizeof($graphs)) {
			foreach ($graphs as $g) {
				...
				//form_selectable_cell(get_permission_string($g, $policies), $g['local_graph_id']); // -- 之前的判断
				if(empty($g["user1"]) && empty($g["user2"])){
				    form_selectable_cell('<span class="accessGranted">' . __('Access Granted') . '</span>', $g['local_graph_id']);
				}else if(empty($g["user1"]) && !empty($g["user2"])){
				    form_selectable_cell('<span class="accessRestricted">' . __('Access Restricted') . ':主机限制</span>', $g['local_graph_id']);
				}else if(!empty($g["user1"]) && empty($g["user2"])){
				    form_selectable_cell('<span class="accessRestricted">' . __('Access Restricted') . ':图形限制</span>', $g['local_graph_id']);
				}else{
	...
	case 'permstr': // 树
		...
		$forms = db_fetch_assoc($sql_query);
    ...
    case 'permstopo': // 拓扑
        ...
        process_topo_request_vars();
        
        topo_filter($header_label);
        
        /*
        form_start('user_admin.php', 'policy');
        
        html_start_box(__('默认拓扑图策略'), '100%', '', '3', 'center', '');
        
        ?>
		<tr class='even'>
			<td><table><tr>
			<td class='nowrap'><?php print __('此用户的默认拓扑图策略');?></td>
			<td>
				<?php form_dropdown('policy_topos',$policy_array,'','',$policy['policy_topos'],'',''); ?>
			</td>
			<td>
				<input type='submit' class='ui-button ui-corner-all ui-widget' name='update_policy' value='<?php print __esc('Update');?>'>
				<input type='hidden' name='tab' value='<?php print $tab;?>'>
		...
		if (get_request_var('associated') == 'true') {
			...
			$sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (user_auth_perms.type=101 AND user_auth_perms.user_id=' . get_request_var('id', 0) . ')';
		}

		$total_rows = db_fetch_cell("SELECT
			COUNT(DISTINCT gt.id)
			FROM plugin_qunee AS gt
			LEFT JOIN user_auth_perms
			ON gt.id = user_auth_perms.item_id
			AND user_auth_perms.type = 101
			AND user_auth_perms.user_id = " . get_request_var('id') . "
			$sql_where");
		...
		$nav = html_nav_bar('user_admin.php?action=user_edit&tab=permstopo&id=' . get_request_var('id'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('拓扑图'), 'page', 'main');

		form_start('user_admin.php?tab=permstopo&id=' . get_request_var('id'), 'chk');

		print $nav;

		html_start_box('', '100%', '', '3', 'center', '');

		$display_text = array(__('拓扑名称'), __('ID'), __('Effective Policy'));
		...
		if (cacti_sizeof($forms)) {
			...
			print '<tr><td colspan="' . (cacti_sizeof($display_text)+1) . '"><em>' . __('无数据') . '</em></td></tr>';
		}
		...
		form_hidden_box('tab',$tab,'');
		form_hidden_box('id', get_request_var('id'), '');
		form_hidden_box('associate_topo', '1', '');
    ...
    case 'permsrep': // 报告
        ...
        process_rep_request_vars();
        
        rep_filter($header_label);
        
        /*
        form_start('user_admin.php', 'policy');
        
        html_start_box(__('默认报告策略'), '100%', '', '3', 'center', '');
        
        ?>
		<tr class='even'>
			<td><table><tr>
			<td class='nowrap'><?php print __('此用户的默认报告策略');?></td>
			<td>
				<?php form_dropdown('policy_reps',$policy_array,'','',$policy['policy_reps'],'',''); ?>
			</td>
			<td>
				<input type='submit' class='ui-button ui-corner-all ui-widget' name='update_policy' value='<?php print __esc('Update');?>'>
				<input type='hidden' name='tab' value='<?php print $tab;?>'>
		...
		if (get_request_var('associated') == 'true') {
			...
			$sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (user_auth_perms.type=102 AND user_auth_perms.user_id=' . get_request_var('id', 0) . ')';
		}
		...
		$nav = html_nav_bar('user_admin.php?action=user_edit&tab=permsrep&id=' . get_request_var('id'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('拓扑图'), 'page', 'main');

		form_start('user_admin.php?tab=permsrep&id=' . get_request_var('id'), 'chk');

		print $nav;

		html_start_box('', '100%', '', '3', 'center', '');

		$display_text = array(__('报告名称'), __('ID'), __('Effective Policy'));
		...
		form_hidden_box('tab',$tab,'');
		form_hidden_box('id', get_request_var('id'), '');
		form_hidden_box('associate_rep', '1', '');
    ...
    case 'permsform': // 报表管理
        ...
        process_form_request_vars();
        
        form_filter($header_label);
        /*
        form_start('user_admin.php', 'policy');
        
        html_start_box(__('默认报表管理策略'), '100%', '', '3', 'center', '');
        
        ?>
		<tr class='even'>
			<td><table><tr>
			<td class='nowrap'><?php print __('此用户的默认报表管理策略');?></td>
			<td>
				<?php form_dropdown('policy_forms',$policy_array,'','',$policy['policy_forms'],'',''); ?>
			</td>
			<td>
				<input type='submit' class='ui-button ui-corner-all ui-widget' name='update_policy' value='<?php print __esc('Update');?>'>
				<input type='hidden' name='tab' value='<?php print $tab;?>'>
		...
		if (get_request_var('types') != '-1') {
		    $sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . " (gt.type = " . get_request_var('types') . ")";
		}

		if (get_request_var('associated') == 'true') {
			/* showing all rows */
		} else {
			$sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (user_auth_perms.type=103 AND user_auth_perms.user_id=' . get_request_var('id', 0) . ')';
		}

		$total_rows = db_fetch_cell("SELECT
			COUNT(DISTINCT gt.id)
			FROM view_plugin_report AS gt
			LEFT JOIN user_auth_perms
			ON gt.id = user_auth_perms.item_id
			AND user_auth_perms.type = 103
			AND user_auth_perms.user_id = " . get_request_var('id') . "
			$sql_where");

		$sql_query = "SELECT gt.id, gt.name,gt.type, user_auth_perms.user_id
			FROM view_plugin_report AS gt
			LEFT JOIN user_auth_perms
			ON gt.id = user_auth_perms.item_id
			AND user_auth_perms.type = 103
		...
		//cacti_log("sql_where = " . $sql_query);	
		$forms = db_fetch_assoc($sql_query);

		$nav = html_nav_bar('user_admin.php?action=user_edit&tab=permsform&id=' . get_request_var('id'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('拓扑图'), 'page', 'main');

		form_start('user_admin.php?tab=permsform&id=' . get_request_var('id'), 'chk');

		print $nav;

		html_start_box('', '100%', '', '3', 'center', '');

		$display_text = array(__('报表名称'), __('类型'), __('ID'), __('Effective Policy'));
		...
		if (cacti_sizeof($forms)) {
			foreach ($forms as $t) {
				form_alternate_row('line' . $t['id'], true);
				form_selectable_cell(filter_value($t['name'], get_request_var('filter')), $t['id']);
				form_selectable_cell($form_types[$t['type']], $t['id']);
				form_selectable_cell($t['id'], $t['id']);
				if (empty($t['user_id']) || $t['user_id'] == NULL) {
					if ($policy['policy_forms'] == 1) {
						form_selectable_cell('<span class="accessGranted">' . __('Access Granted') . '</span>', $t['id']);
					} else {
						form_selectable_cell('<span class="accessRestricted">' . __('Access Restricted') . '</span>', $t['id']);
					}
		...
		form_hidden_box('tab',$tab,'');
		form_hidden_box('id', get_request_var('id'), '');
		form_hidden_box('associate_form', '1', '');
...
function user_realms_edit($header_label) {
	...
	// 查询当前用户是否保存过
	$is_realm_perms_saved = db_fetch_cell_prepared('SELECT is_realm_perms_saved
					FROM user_auth WHERE id = ?',
					array(get_request_var('id', 0)));
	...
	foreach($user_auth_roles as $role_name => $perms) {
		...
		foreach($perms as $realm) {
			...
			if (isset($user_auth_realms[$realm])) {
			    $realmNames = explode("_", $user_auth_realms[$realm]);
				...
				if (!empty($set)) {
				    ...
				    if(empty($is_realm_perms_saved) && sizeof($realmNames) == 2 && $realmNames[1] == "1"){
    					$old_value = 'on';
				    }else{
    					$old_value = '';
				    }
				...
				form_checkbox('section' . $realm, $old_value, $realmNames[0], '', '', '', (!isempty_request_var('id') ? 1 : 0)); print '<br>';
			}
			print "</td>\n";
...
function user() {
	...
	$buttons = array(
	    array(
	        'href'     => 'user_admin.php?tab=general&action=user_edit',
	        'callback' => true,
	        'title'    => "添加",
	        'class'    => 'fa fa-plus'
	    ),
	    array(
	        'href'     => 'user_admin.php?action=import',
	        'callback' => true,
	        'title'    => "导入",
	        'class'    => 'fa fa-upload'
	    )
	);
	html_start_box(__('User Management'), '100%', '', '3', 'center', $buttons);
	...
	$user_list = db_fetch_assoc("SELECT user_auth.id, user_auth.username, user_auth.full_name,
		user_auth.realm, user_auth.enabled, user_auth.policy_graphs, user_auth.policy_hosts, user_auth.policy_graph_templates,
		time, max(time) as dtime,uag.name as group_name
		FROM user_auth
        left join user_auth_group_members uagm on uagm.user_id = user_auth.id
        left join user_auth_group uag on uagm.group_id = uag.id
		LEFT JOIN user_log ul ON (user_auth.id = ul.user_id)
		$sql_where
		GROUP BY id
		$sql_order
		$sql_limit");
	...
	if (cacti_sizeof($user_list)) {
		foreach ($user_list as $user) {
			if (empty($user['dtime']) || ($user['dtime'] == '12/31/1969')) {
				...
				$last_login = strftime('%Y-%m-%d %H:%M:%S', strtotime($user['dtime']));;
			}
			if ($user['enabled'] == 'on') {
				$enabled = __('Yes');
			} else {
			...
			form_selectable_cell($user['group_name'], $user['id']);
...
function process_topo_request_vars() {
	/* ================= input validation and session storage ================= */
	$filters = array(
		'rows' => array(
			'filter' => FILTER_VALIDATE_INT,
	...
	validate_store_request_vars($filters, 'sess_uatopo');
	/* ================= input validation ================= */
}

function process_rep_request_vars() {
	/* ================= input validation and session storage ================= */
	$filters = array(
		'rows' => array(
			'filter' => FILTER_VALIDATE_INT,
	...
	validate_store_request_vars($filters, 'sess_uarep');
	/* ================= input validation ================= */
}
// 报表
function process_form_request_vars() {
	/* ================= input validation and session storage ================= */
	$filters = array(
		'rows' => array(
			'filter' => FILTER_VALIDATE_INT,
	...
	validate_store_request_vars($filters, 'sess_uaform');
	/* ================= input validation ================= */
}
...
function topo_filter($header_label) {
    global $config, $item_rows;
	...
	function applyFilter() {
		strURL  = 'user_admin.php?action=user_edit&tab=permstopo&id=<?php print get_request_var('id');?>'
		strURL += '&rows=' + $('#rows').val();
		strURL += '&associated=' + $('#associated').is(':checked');
		strURL += '&filter=' + $('#filter').val();
		strURL += '&header=false';
	...
	function clearFilter() {
		strURL = 'user_admin.php?action=user_edit&tab=permstopo&id=<?php print get_request_var('id');?>&clear=true'
		strURL = strURL + '&header=false';
		loadPageNoHeader(strURL);
	}
	...
	html_start_box(__('拓扑图权限 %s', html_escape($header_label)), '100%', '', '3', 'center', '');
	...
	<tr class='even'>
		...
		<form id='forms' method='post' action='user_admin.php'>
			...
			<input type='hidden' name='action' value='user_edit'>
			<input type='hidden' name='tab' value='permstopo'>
			<input type='hidden' name='id' value='<?php print get_request_var('id');?>'>
		</form>
		</td>
	</tr>
...
// 报告
function rep_filter($header_label) {
    global $config, $item_rows;
	...
	function applyFilter() {
		strURL  = 'user_admin.php?action=user_edit&tab=permsrep&id=<?php print get_request_var('id');?>'
		strURL += '&rows=' + $('#rows').val();
		strURL += '&associated=' + $('#associated').is(':checked');
		strURL += '&filter=' + $('#filter').val();
		strURL += '&header=false';
	...
	function clearFilter() {
		strURL = 'user_admin.php?action=user_edit&tab=permsrep&id=<?php print get_request_var('id');?>&clear=true'
		strURL = strURL + '&header=false';
		loadPageNoHeader(strURL);
	}
	...
	html_start_box(__('报告权限 %s', html_escape($header_label)), '100%', '', '3', 'center', '');
	...
	<tr class='even'>
		...
		<form id='forms' method='post' action='user_admin.php'>
			...
			<input type='hidden' name='action' value='user_edit'>
			<input type='hidden' name='tab' value='permsrep'>
			<input type='hidden' name='id' value='<?php print get_request_var('id');?>'>
		</form>
		</td>
	</tr>
...
// 报表管理
function form_filter($header_label) {
    global $config, $item_rows,$form_types;
    
    ?>
	<script type='text/javascript'>

	function applyFilter() {
		strURL  = 'user_admin.php?action=user_edit&tab=permsform&id=<?php print get_request_var('id');?>'
		strURL += '&rows=' + $('#rows').val();
		strURL += '&types=' + $('#types').val();
		strURL += '&associated=' + $('#associated').is(':checked');
		strURL += '&filter=' + $('#filter').val();
		strURL += '&header=false';
		loadPageNoHeader(strURL);
	...
	function clearFilter() {
		strURL = 'user_admin.php?action=user_edit&tab=permsform&id=<?php print get_request_var('id');?>&clear=true'
		strURL = strURL + '&header=false';
		loadPageNoHeader(strURL);
	}
	...
	html_start_box(__('报表管理权限 %s', html_escape($header_label)), '100%', '', '3', 'center', '');
		...
		<form id='forms' method='post' action='user_admin.php'>
			<table class='filterTable'>
				<tr>
					...
					<td>
						<select id='types' onChange='applyFilter()'>
							<option value='-1'<?php print (get_request_var('types') == '-1' ? ' selected>':'>') . __('全部');?></option>
							<?php
							if (cacti_sizeof($form_types)) {
							    foreach ($form_types as $key => $value) {
									print "<option value='" . $key . "'"; if (get_request_var('types') == $key) { print ' selected'; } print '>' . html_escape($value) . "</option>\n";
								}
							}
							?>
						</select>
			...
			<input type='hidden' name='action' value='user_edit'>
			<input type='hidden' name='tab' value='permsform'>
			<input type='hidden' name='id' value='<?php print get_request_var('id');?>'>
		</form>
		</td>
	</tr>
...
function import_user(){
    if(!empty(get_request_var("save_component_site"))){ // 如果是保存方法，下面开始上传
            ...
            if ($_FILES["file"]["type"] == "application/vnd.ms-excel" && in_array($extension, $allowedExts)){
                if ($_FILES["file"]["error"] > 0){
                    ...
                    while(($value = fgetcsv($handle)) !== FALSE){
                        ...
                        if(sizeof($value) != 4){
                            raise_message(2,"用户导入列数量不正确",MESSAGE_LEVEL_ERROR);
                            header('Location: user_admin.php?action=import');
                            exit();
                        }
                        $hs = db_fetch_row_prepared('SELECT * FROM user_auth WHERE username = ? limit 1',array($value[0]));
                        if (!empty($hs)) { // 说明名称重复
                            continue;
                        }
                        $save = array();
                        $save['id']                   = get_nfilter_request_var('id',"0");
                        $save['username']             = form_input_validate($value[0], 'username', "^[A-Za-z0-9\._\\\@\ -]+$", false, 3);
                        $save['full_name']            = form_input_validate($value[1], 'full_name', '', true, 3);
                        $save['password']             = compat_password_hash($value[3], PASSWORD_DEFAULT);
                        $save['must_change_password'] = form_input_validate(get_nfilter_request_var('must_change_password', ''), 'must_change_password', '', true, 3);
                        $save['password_change']      = form_input_validate(get_nfilter_request_var('password_change', 'on'), 'password_change', '', true, 3);
                        $save['show_tree']            = form_input_validate(get_nfilter_request_var('show_tree', 'on'), 'show_tree', '', true, 3);
                        $save['show_list']            = form_input_validate(get_nfilter_request_var('show_list', 'on'), 'show_list', '', true, 3);
                        $save['show_preview']         = form_input_validate(get_nfilter_request_var('show_preview', 'on'), 'show_preview', '', true, 3);
                        $save['graph_settings']       = form_input_validate(get_nfilter_request_var('graph_settings', 'on'), 'graph_settings', '', true, 3);
                        $save['login_opts']           = form_input_validate(get_nfilter_request_var('login_opts',"1"), 'login_opts', '', true, 3);
                        $save['realm']                = get_nfilter_request_var('realm', 0);
                        $save['password_history']     = "";
                        $save['enabled']              = form_input_validate(get_nfilter_request_var('enabled', 'on'), 'enabled', '', true, 3);
                        $save['email_address']        = form_input_validate(get_nfilter_request_var('email_address', $value[2]), 'email_address', '', true, 3);
                        $save['locked']               = form_input_validate(get_nfilter_request_var('locked', ''), 'locked', '', true, 3);
                        $save['reset_perms']          = mt_rand();
                        if ($save['locked'] == '') {
                            $save['failed_attempts'] = 0;
        ...
        top_header();
        form_start('user_admin.php', 'user_admin_import',true);
        $buttons = array(
            array(
                'href'     => 'user_admin.php?action=dl_tpl',
                'callback' => false,
                'title'    => "下载CSV模板",
                'class'    => 'fa fa-download'
            )
        );
        html_start_box("导入用户", '100%', '', '3', 'center', $buttons);
        draw_edit_form(
            array(
                'config' => array('no_form_tag' => true),
		...
		$(function() {
			$("input[id^='btn_']").click(function(){
				...
				if(value == "ret"){
					cactiReturnTo("user_admin.php");
				}else if(!checkFile()){
    				alert("请先选择要上传的文件！");
    				return;
				}else{
    				$("#user_admin_import").append("<input type='hidden' name='op' value='"+value+"'>");
    				$("#user_admin_import").submit();
				}
			});
		});
		function checkFile(){
