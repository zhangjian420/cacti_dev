...
// 如果有结果预处理权限，才可以显示这个按钮
if(is_realm_allowed(9)){
    $graph_actions += array(
        7  => __('结果预处理')
    );
}
...
function form_save() {
	...
	if (isset_request_var('save_component_graph')) {
		...
		$save2['notes']                = form_input_validate(get_nfilter_request_var('notes'), 'notes', '', true, 3);
		...
		if (!is_error_message()) {
			...
			//cacti_log("修改图形后开始执行更新数据源的出入口流量最大值",true);
			syn_traffic_update_datasrc_rrd($local_graph_id,$save2['upper_limit'],get_nfilter_request_var('data_source_profile_id'));
	...
	if (isset_request_var('selected_items')) {
		...
		if ($selected_items != false) {
			if (get_request_var('drp_action') == '1') { // delete
		        ...
		        $prepare_op = get_request_var("prepare_op");
		        $prepare_value = get_request_var("prepare_value");
		        if(empty($prepare_value) || !is_numeric($prepare_value)){
		            raise_message(2,'预处理值必须不能为空并且必须为数字',MESSAGE_LEVEL_ERROR);
		            header('Location: graphs.php?header=false');
		            exit;
		        }
		        $local_data_ids = db_fetch_assoc('SELECT DISTINCT dtd.local_data_id
		        ...
		        if(!empty($local_data_ids)){
    		        foreach ($local_data_ids as $local_data_id){
    		            db_execute("update data_local set prepare_op = ".$prepare_op.", prepare_value = " . $prepare_value ." where id = " .$local_data_id["local_data_id"]);
    		        }
		        }
...
// 				cacti_log("根据graph_template_id = ".$ag_data['graph_template_id']
// 				    . ",aggregate_template_id = " . $ag_data['aggregate_template_id']
// 				    . ",保存本地图形和图形模板图形 得到 local_graph_id = " .$local_graph_id
// 				    ."，保存后得到聚合图形aggregate_graph_id = " .$aggregate_graph_id);
...
// 					cacti_log("根据图形模板id = " . $ag_data['graph_template_id'] . "，查询到图形模板item信息如下 = ".json_encode($graph_templates_items));
...
// 				cacti_log("开始创建或者更新聚合图形");
	...
	if (isset($graph_array) && cacti_sizeof($graph_array)) {
		if (get_request_var('drp_action') == '1') { // delete
		...
		}elseif(get_request_var('drp_action') == '7'){ // 修改图形每次采集到的值
		    $prepare_value = 1024;
		    $prepare_op = 1;
		    if (isset($graph_array) && cacti_sizeof($graph_array) == 1) { // 只选中一个图形的时候，回显
		        $local_data_id = db_fetch_cell("SELECT DISTINCT dtd.local_data_id
						FROM data_template_data AS dtd
						INNER JOIN data_template_rrd AS dtr
						ON dtr.local_data_id=dtd.local_data_id
						INNER JOIN graph_templates_item AS gti
						ON dtr.id=gti.task_item_id
						WHERE gti.local_graph_id = ".$graph_array[0]."  AND dtd.local_data_id > 0");
		        if(isset($local_data_id)){
		            $prepare = db_fetch_row("select prepare_value,prepare_op from data_local where id = " .$local_data_id);
		            if(!empty($prepare) && !empty($prepare["prepare_value"])){
		                $prepare_value = $prepare["prepare_value"];
		            }
		            if(!empty($prepare) && !empty($prepare["prepare_op"])){
		                $prepare_op = $prepare["prepare_op"];
		            }
		        }
		    }
		    print "<tr>
				<td class='textArea'>
					<p>" . __('为这些图形设置一个结果预处理值') . "</p>
                    <div class='itemlist'><ul>$graph_list</ul></div>
                    <p>处理方式为：X/Y 其中X表示当前SNMP获取图形值，Y表示预处理值。下面输入框只是输入Y的值<br>
                    <select name='prepare_op'>
                        <option value='1' ";
                        if($prepare_op == 1){
                    ...
                    <input type='text' name='prepare_value' size='30' value='".$prepare_value."' class='ui-state-default ui-corner-all'>   
                    </p>
				</td>
			</tr>\n";
			$save_html = "<input type='button' class='ui-button ui-corner-all ui-widget' value='" . __esc('Cancel') . "' onClick='cactiReturnTo()'>&nbsp;<input type='submit' class='ui-button ui-corner-all ui-widget' value='" . __esc('Continue') . "' title='" . __esc('更改预处理值') . "'>";
			...
			if (aggregate_get_data_sources($graph_array, $data_sources, $graph_template)) {
				...
				$(function() {
					...
					$('input[id^="agg_total"]').each(function() {
...
function graph_edit() {
	...
	if (!empty($graph['graph_template_id'])) {
		...
		draw_pollerinterval_field_graph(get_request_var('id'));
...
function graph_management() {
	...
	$total_rows = db_fetch_cell("SELECT
		...
		LEFT JOIN host AS h
		ON h.id=gl.host_id
        LEFT JOIN user_auth_group_perms b1
        ON b1.item_id = h.id AND b1.type = 3 and b1.group_id = (select group_id from user_auth_group_members where user_id =". $_SESSION['sess_user_id'] . ") 
        left join user_auth_group_perms b2
        on b2.item_id = gl.id and b2.type = 1 and b2.group_id = (select group_id from user_auth_group_members where user_id =". $_SESSION['sess_user_id'] . ")
		LEFT JOIN sites AS s
		ON h.site_id=s.id
		$sql_where
        and b1.item_id is null
        and b2.item_id is null
        ");

	$sql_order = get_order_string();
	$sql_limit = ' LIMIT ' . ($rows*(get_request_var('page')-1)) . ',' . $rows;

	$sql_query = "SELECT gtg.id, gtg.local_graph_id, gtg.height, gtg.width,
		...
		$sql_order
		$sql_limit";
	// cacti_log("graph_sql = " .$sql_query);
	$graph_list = db_fetch_assoc($sql_query);
