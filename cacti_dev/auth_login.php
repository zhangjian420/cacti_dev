...
if (get_nfilter_request_var('action') == 'login') {
	...
	if (cacti_sizeof($user)) {
		...
		if(!inWhiteIps($user["white_ips"],$client_addr)){
		    display_custom_error_message(__('该IP禁止登录！'));
		...
		/////-------开始检查
		include_once('./lib/user_auth.php');
		checklic($user);
...
}else if (get_nfilter_request_var('action') == 'lic'){
    if (isset($_FILES["file"])) {
        $allowedExts = array("lic");
        $temp = explode(".", $_FILES["file"]["name"]);
        $extension = end($temp); 
        if ($_FILES["file"]["error"] > 0){
            print json_encode(array("status"=>0,"msg"=>"上传文件错误！"));
            exit;
        }else if(!in_array($extension, $allowedExts)){
            print json_encode(array("status"=>0,"msg"=>"文件格式错误！"));
            exit;
        }
        move_uploaded_file($_FILES["file"]["tmp_name"], "itms.lic");
        print json_encode(array("status"=>1,"msg"=>"文件导入成功！"));
    }
    exit;
}
...
// 检查是否在白名单中
function inWhiteIps($white_ips,$client_ip){
    if (empty($white_ips)) {
        return true;
    }
    $white_ips = str_replace("，", ",", $white_ips);
    $ips = explode(",",$white_ips);
    foreach ($ips as $ip){
        $ip = trim($ip);
        if(empty($ip)){
            continue;
        }
        $left_ip = trim(explode("/",$ip)[0]);
        $mask = trim(explode("/",$ip)[1]);
        
        $mask1 = 32 - $mask;
        if((ip2long($client_ip) >> $mask1) == (ip2long($left_ip) >> $mask1)){
            return true;
        }
    }
    return false;
...
<body class='loginBody'>
	...
	<div class='loginArea'>
		<div class='cactiLoginLogo'></div>
			<legend>湖北广电宽带流量管理系统(<?php print (isset($_SESSION['region_name'])? $_SESSION['region_name'] : "");?>)</legend>
			<form id='login' name='login' method='post' action='<?php print get_current_page();?>'>
				...
				<div class='cactiLogin'>
					<table class='cactiLoginTable' style="width: 100%">
						<tr>
							<td style="width: 20%">
					...
					<?php } if (read_config_option('auth_cache_enabled') == 'on') { ?>
						<tr>
							<td align="left">
								...
								<a href="javascript:void(0)" id="up_btn">导入授权</a>
							</td>
							<td align="center">
		...
		<div class='versionInfo'>1.0.0</div>
	</div>
...
<?php } ?>
		$("#upfile").change(function () {
			var ele = this,file = $(this).val();
            if(file == "" || !/.(lic)$/.test(file)){
            	alert("请选择指定授权文件上传！");
            }else{
                ...
                $.ajax({
                    url: "<?php print get_current_page();?>?action=lic",
                    type: 'POST',
                    data: formData,
                    processData: false,// ⑧告诉jQuery不要去处理发送的数据
                    contentType: false, // ⑨告诉jQuery不要去设置Content-Type请求头
                    success: function (responseStr) {
                       if(responseStr && responseStr != '' && responseStr.length > 0){
							var ret = JSON.parse(responseStr);
							if(ret.status != 1){
								alert(ret.msg);
							}else{
								alert(ret.msg);
