...
function host_new_graphs_save($host_id) {
	...
	if (isset($_SESSION['sess_graph_num_lic']) && ($graph_num + $numing - 1) >= $_SESSION['sess_graph_num_lic']) {
	    raise_message(1001,"图形最多创建".$_SESSION['sess_graph_num_lic']."个，若增加图形请重新授权！",MESSAGE_LEVEL_ERROR);
	    header('Location: graphs_new.php');
	    exit;
	}
					...
					foreach ($snmp_query_graphs as $snmp_query_graph) {
						...
						if (cacti_sizeof($created_graphs)) {
							foreach ($created_graphs as $created_graph) {
							    //cacti_log("snmp_index = ".trim($created_graph['snmp_index']) . ",md5 = ". encode_data_query_index(trim($created_graph['snmp_index'])));
							    $script .= ($cg_ctr > 0 ? ',' : '') . "'" . encode_data_query_index(trim($created_graph['snmp_index'])) . "'";
								++$cg_ctr;
							}
						}
