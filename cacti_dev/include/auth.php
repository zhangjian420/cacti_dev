...
// 查询系统属于省还是某个地市
$rname = db_fetch_cell("select r.name from plugin_overview po left join region r on po.system_region = r.code limit 1 ");
if(!empty($rname)){
    $_SESSION['region_name'] = $rname;
}else{
    $_SESSION['region_name'] = "区县";
}
...
if (read_config_option('auth_method') == 0) {
	...
	$_SESSION['sess_user_username'] = read_config_option('admin_user');
...
if (read_config_option('auth_method') != 0) {
	...
	if (!isset($_SESSION['sess_user_id'])) {
		...
		if ($cookie_user !== false) {
		    ...
		    checklic($cookie_user);
		    
			$_SESSION['sess_user_id'] = $cookie_user["id"];
			$_SESSION['sess_user_username'] = $cookie_user["username"];
		}
	}
	...
	if (empty($_SESSION['sess_user_id'])) {
		...
		/* Are we upgrading from a version before 1.2 which has the Install/Upgrade realm 26  这个权限都变化了，系统里面不存在了*/
		if ($realm_id == 26) {
		...
		if ($realm_id > 0) {
			...
			//cacti_log($auth_sql_query . " ==== " . json_encode($auth_sql_params));
			$authorized = db_fetch_cell_prepared($auth_sql_query, $auth_sql_params);
		} else {
			$authorized = false;
		}
		//cacti_log($realm_id . " = " . $authorized);
		if ($realm_id != -1 && !$authorized) {
			...
			$title_body = '<p>' . __('没有访问权限。') . '</p><p>' . __('如果有疑问，请联系管理员');
