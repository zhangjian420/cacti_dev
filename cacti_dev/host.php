...
include_once('./lib/host_import.php');
...
function add_tree_names_to_actions_array() {
	...
	if (cacti_sizeof($trees)) {
		foreach ($trees as $tree) {
			$device_actions['tr_' . $tree['id']] = '放在树上 (' . $tree['name'] . ')';
		}
	}
}
...
function form_save() {
	if (isset_request_var('save_component_host')) {
		if (get_nfilter_request_var('snmp_version') == 3 && (get_nfilter_request_var('snmp_password') != get_nfilter_request_var('snmp_password_confirm'))) {
			...
			if (isset($_SESSION['sess_host_num_lic']) && ($host_num) >= $_SESSION['sess_host_num_lic'] && empty(get_request_var("id"))) {
			    raise_message(1002,"主机最多创建".$_SESSION['sess_host_num_lic']."个，若增加主机请重新授权！",MESSAGE_LEVEL_ERROR);
			...
			if($host_id == -1){
			    raise_message(2,"主机名重复，插入失败",MESSAGE_LEVEL_ERROR);
			    header("Location: host.php?header=false");
			    exit();
			}else{
			    if ($host_id !== false) {
		...
		cacti_logdb((empty(get_nfilter_request_var('id')) ? "添加主机" : "修改主机") .trim(get_nfilter_request_var('hostname')));
...
function get_device_records(&$total_rows, $rows) {
	...
	$sql_join = "LEFT JOIN user_auth_group_perms c on c.item_id = host.id and c.type = 3 ";
	$sql_join .= "and c.group_id = (select group_id from user_auth_group_members where user_id =". $_SESSION['sess_user_id'] . ") ";
	$sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' c.item_id is null';
...
function host() {
	...
	html_start_box(__('Devices'), '100%', '', '3', 'center', $buttons);
...
function import_host(){
    if(!empty(get_request_var("save_component_site"))){ // 如果是保存方法，下面开始上传
                    ...
                    while(($value = fgetcsv($handle)) !== FALSE){
                        ...
                        if($op == "imp" || $op == "edit"){
                            if($op == "imp"){ // 如果是添加
                                // check
                                $host_num = db_fetch_cell("select count(*) from host");
                                if (isset($_SESSION['sess_host_num_lic']) && ($host_num) >= $_SESSION['sess_host_num_lic']) {
                                    raise_message(1002,"主机最多创建".$_SESSION['sess_host_num_lic']."个，若增加主机请重新授权！",MESSAGE_LEVEL_ERROR);
                                    header('Location: host.php?action=import');
                                    exit;
                                }
                                if(sizeof($value) != 3){
                                    raise_message(2,"'从导入中添加'列数量不正确",MESSAGE_LEVEL_ERROR);
                                    header('Location: host.php?action=import');
                                    exit();
                                }
                                $description = $value[0];
                                $hostname = $value[1];
                                $snmp_community = $value[2];
                                $id = "0";
                                //cacti_log("添加 : id = $id ,description = " . $description .",hostname = ".$hostname .",snmp_community = " . $snmp_community);
                            }else{ // 如果是修改
                                if(sizeof($value) != 2){
                                    raise_message(2,"'从导入中修改'列数量不正确",MESSAGE_LEVEL_ERROR);
                                    header('Location: host.php?action=import');
                                    exit();
                                }
                                $hostname = $value[0];
                                $snmp_community = $value[1];
                                $hs = db_fetch_row_prepared('SELECT id,description FROM host WHERE hostname = ? limit 1',array($value[0]));
                                if (empty($hs)) {
                                    continue;
                                }
                                $id = $hs["id"];
                                $description = $hs["description"];
                                //cacti_log("修改 : id = $id ,description = " . $description .",hostname = ".$hostname .",snmp_community = " . $snmp_community);
                            }
                            api_device_save(
                                get_nfilter_request_var('id',$id),
                                get_nfilter_request_var('host_template_id',"7"),
                                get_nfilter_request_var('description',$description),
                                trim(get_nfilter_request_var('hostname',$hostname)),
                                get_nfilter_request_var('snmp_community',$snmp_community),
                            ...
                            if(sizeof($value) != 1){
                                raise_message(2,"'从导入中删除'列数量不正确",MESSAGE_LEVEL_ERROR);
                                header('Location: host.php?action=import');
                                exit();
                            }
                            $host_id = db_fetch_cell_prepared('SELECT id FROM host WHERE hostname = ? limit 1',array($value[0]));
                            if (!empty($host_id)) {
                                $del_arr[] = $host_id;
                            }
                        }
                    }
                    if($op == "del" && !empty($del_arr)){
                        header('Location: host.php?delete_type=2&action=actions&drp_action=1&selected_items='.serialize($del_arr));
                        exit();
                    }
                    fclose($handle);
                    header('Location: host.php');
        ...
        top_header();
        form_start('host.php', 'host_import',true);
        $buttons = array(
            array(
                'href'     => 'host.php?action=dl_tpl',
                'callback' => false,
                'title'    => "下载CSV模板",
                'class'    => 'fa fa-download'
            )
        );
        html_start_box("导入设备", '100%', '', '3', 'center', $buttons);
        draw_edit_form(
            array(
                'config' => array('no_form_tag' => true),
		...
		$(function() {
			$("input[id^='btn_']").click(function(){
				...
				if(value == "ret"){
					cactiReturnTo("host.php");
				}else if(!checkFile()){
    				alert("请先选择要上传的文件！");
    				return;
				}else{
    				$("#host_import").append("<input type='hidden' name='op' value='"+value+"'>");
    				$("#host_import").submit();
				}
			});
		});
