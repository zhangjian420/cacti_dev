...
function form_save() {
	...
	if (isset_request_var('save_component_tree')) {
		...
		if (!is_error_message()) {
		    // 更新该用户修改锁定的树为不锁定的状态。
		    db_execute_prepared('UPDATE graph_tree
		      SET `locked` = 0, last_modified = NOW() WHERE modified_by = ? and `locked` = 1',
		...
		cacti_logdb((empty(get_request_var('id')) ? "添加数" : "修改树").$save['name']);
...
function tree_edit() {
	...
	if (isset($tree['locked']) && $tree['locked'] == 0) {
		...
		if ($tree['modified_by'] == $_SESSION['sess_user_id']) {
			$editable = true;
		    $lockdiv = "<div style='padding:3px;'><table><tr>
                    <td><input type='button' class='ui-button ui-corner-all ui-widget' id='unlock' value='" . __esc('Finish Editing Tree') . "'></td>
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
                    <td><input type='button' class='ui-button ui-corner-all ui-widget' id='addbranch' value='" . __esc('Add Root Branch') . "' onClick='createNode()'></td>";
		    $lockdiv .= '<td><input type="button" class="ui-button ui-corner-all ui-widget" onclick="treeNodeMoveClick(\'unsel\')" value="取消选中"></td>';
		    $lockdiv .= '<td><input type="button" class="ui-button ui-corner-all ui-widget" onclick="treeNodeMoveClick(\'on\')" value="添加到树上"></td>';
		    $lockdiv .= '<td><input type="button" class="ui-button ui-corner-all ui-widget" onclick="treeNodeMoveClick(\'before\')" value="向上移动"></td>';
		    $lockdiv .= '<td><input type="button" class="ui-button ui-corner-all ui-widget" onclick="treeNodeMoveClick(\'after\')" value="向下移动"></td>';
		    $lockdiv .= '<td><input type="button" class="ui-button ui-corner-all ui-widget" onclick="treeNodeMoveClick(\'del\')" value="删除选中"></td>';
		    $lockdiv .= "<td style='font-weight:bold;'>" . __('This tree has been locked for Editing on %1$s by %2$s.', $tree['locked_date'], get_username($tree['modified_by']));
	...
	if (!isempty_request_var('id')) {
		...
		function treeNodeMoveClick(direct){
			var ctree = $("#ctree").jstree(true);
			if(direct == "on"){
				var graphs = $("#graphs").jstree(true);
				var selectedGraphsNodes = graphs.get_selected(true);
				if(selectedGraphsNodes.length == 0){
					alert("请选择可用的图形！");
					return;
				}
				var selectedCtreeNodes = ctree.get_selected(true);
				var selectedCtreeNode;
				if(selectedCtreeNodes[0] && selectedCtreeNodes[0].type != "default"){
					alert("只能将图形放入到文件夹或者根节点中，不能放到图形上！");
					ctree.deselect_all();	
					return;
				}else{
					selectedCtreeNode = selectedCtreeNodes[0];
				}
				ctree.move_node(selectedGraphsNodes,selectedCtreeNode ? selectedCtreeNode : "#");
			}else if(direct == "before" || direct == "after"){
				var selectedNodes = ctree.get_selected(true);
				if(selectedNodes.length != 1){
					alert("每次只能移动一个节点！");
				}else{
					var selectedNode = selectedNodes[0];
    				var parentId = (selectedNode.parent && selectedNode.parent != "#") ? selectedNode.parents[1] : "#";
					ctree.move_node(selectedNode,parentId,direct);
				}
			}else if(direct == "del"){
				var r=confirm("确定删除选中节点");
				var selectedNodes = ctree.get_selected(true);
				if(r){
					ctree.delete_node(selectedNodes);						
				}
			}else if(direct == "unsel"){
				ctree.deselect_all();	
			}
		}
...
function display_graphs() {
	...
	$query_sql = "SELECT
        ...
        on gl.host_id = ht.id
        LEFT JOIN user_auth_perms b1
        ON b1.item_id = ht.id AND b1.type = 3 and b1.user_id = ".$_SESSION['sess_user_id']."
        left join user_auth_perms b2
        on b2.item_id = gl.id and b2.type = 1 and b2.user_id = ".$_SESSION['sess_user_id']."
		$sql_where
        and b1.item_id is null
        and b2.item_id is null
		ORDER BY title_cache
		LIMIT ".read_config_option('autocomplete_rows');
	//cacti_log("g1 = " . $query_sql);
	$graphs = db_fetch_assoc($query_sql);
...
function tree() {
	...
	$sql_join = "LEFT JOIN user_auth_group_perms b on b.item_id = t.id and b.type = 2 
        and b.group_id = (select group_id from user_auth_group_members where user_id =". $_SESSION['sess_user_id'] . ") ";
	$sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' b.item_id is null';
	...
	$total_rows = db_fetch_cell("SELECT COUNT(t.id)
		FROM graph_tree AS t
        $sql_join
		$sql_where");
