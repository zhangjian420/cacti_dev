...
function form_actions() {
	...
	if (isset_request_var('associate_host')) {
		...
		exit;
	}elseif (isset_request_var('associate_topo')) { // 关联拓扑
	    foreach ($_POST as $var => $val) {
	        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
	            /* ================= input validation ================= */
	            input_validate_input_number($matches[1]);
	    ...
	    header('Location: user_group_admin.php?action=edit&header=false&tab=permstopo&id=' . get_nfilter_request_var('id'));
	    exit;
	}elseif (isset_request_var('associate_rep')) { // 报告权限
	    foreach ($_POST as $var => $val) {
	        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
	            /* ================= input validation ================= */
	            input_validate_input_number($matches[1]);
	    ...
	    header('Location: user_group_admin.php?action=edit&header=false&tab=permsrep&id=' . get_nfilter_request_var('id'));
	    exit;
	}elseif (isset_request_var('associate_form')) { // 报表管理权限
	    foreach ($_POST as $var => $val) {
	        if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
	            /* ================= input validation ================= */
	            input_validate_input_number($matches[1]);
	    ...
	    header('Location: user_group_admin.php?action=edit&header=false&tab=permsform&id=' . get_nfilter_request_var('id'));
	    exit;
...
function form_save() {
	...
	if (isset_request_var('save_component_group')) {
		...
		if (!is_error_message()) {
			...
			if ($group_id) {
			    // 保存成功后，新增组权限-2020.10.9
			    db_execute_prepared('DELETE FROM user_auth_group_realm
                    WHERE group_id = ?', $group_id);
        	    db_execute_prepared('update user_auth_group set is_realm_perms_saved = 1
        			WHERE id = ?', $group_id);
			    $realm_ids = array(3,4,5,7,8,21,22,25,40,41,42);
			    foreach ($realm_ids as $realm_id){
            	    db_execute_prepared('REPLACE INTO user_auth_group_realm (group_id, realm_id) VALUES (?, ?)', $group_id,$realm_id);
			    }
		...
		db_execute_prepared('DELETE FROM user_auth_group_realm 
            WHERE group_id = ?', array(get_filter_request_var('id')));
		db_execute_prepared('update user_auth_group set is_realm_perms_saved = 1
			WHERE id = ?',
...
function perm_remove() {
	...
	} elseif (get_request_var('type') == 'topo') {
	    db_execute_prepared('DELETE FROM user_auth_group_perms WHERE type=101 AND group_id = ? AND item_id = ?', array(get_request_var('group_id'), get_request_var('id')));
	} elseif (get_request_var('type') == 'rep') {
	    db_execute_prepared('DELETE FROM user_auth_group_perms WHERE type=102 AND group_id = ? AND item_id = ?', array(get_request_var('group_id'), get_request_var('id')));
	} elseif (get_request_var('type') == 'form') {
	    db_execute_prepared('DELETE FROM user_auth_group_perms WHERE type=103 AND group_id = ? AND item_id = ?', array(get_request_var('group_id'), get_request_var('id')));
...
function user_group_graph_perms_edit($tab, $header_label) {
	...
	switch($tab) {
		...
		if (get_request_var('associated') == 'true') {
			...
			$sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (uagp1.type = 1 AND uagp1.group_id=' . get_request_var('id', 0) . ')';
		}
		...
		$total_rows = db_fetch_cell("SELECT
			...
			LEFT JOIN user_auth_group_perms as uagp1 
			ON (gtg.local_graph_id = uagp1.item_id AND uagp1.type = 1 AND uagp1.group_id = " . get_request_var('id') . ")
            LEFT JOIN user_auth_group_perms uagp2
			ON (gl.host_id=uagp2.item_id AND uagp2.type = 3 AND uagp2.group_id = " . get_request_var('id') . ")
			$sql_where");

		$sql_query = "SELECT gtg.local_graph_id, gtg.title_cache, uagp1.group_id as group1,uagp2.group_id as group2
			FROM graph_templates_graph AS gtg
            INNER JOIN graph_local AS gl ON gl.id = gtg.local_graph_id 
            LEFT JOIN host AS h ON h.id = gl.host_id 
			LEFT JOIN user_auth_group_perms uagp1
			ON (gtg.local_graph_id=uagp1.item_id AND uagp1.type = 1 AND uagp1.group_id = " . get_request_var('id') . ")
            LEFT JOIN user_auth_group_perms uagp2
			ON (gl.host_id=uagp2.item_id AND uagp2.type = 3 AND uagp2.group_id = " . get_request_var('id') . ")
			$sql_where
	...
	case 'permstopo': // 拓扑 101
	    ...
	    if (get_request_var('associated') == 'true') {
	        ...
	        $sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (user_auth_group_perms.type=101 AND user_auth_group_perms.group_id=' . get_request_var('id', 0) . ')';
	    }
	    ...
	    $total_rows = db_fetch_cell("SELECT
			...
			AND user_auth_group_perms.type = 101
			AND user_auth_group_perms.group_id = " . get_request_var('id') . "
			$sql_where");
	    ...
	    $sql_query = "SELECT gt.id, gt.name, user_auth_group_perms.group_id
			...
			$nav = html_nav_bar('user_group_admin.php?action=edit&tab=permstopo&id=' . get_request_var('id'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('拓扑图'), 'page', 'main');
			
			form_start('user_group_admin.php?tab=permstopo&id=' . get_request_var('id'), 'chk');
	...
	case 'permsrep': // 报告 102
	    ...
	    if (get_request_var('associated') == 'true') {
	        ...
	        $sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (user_auth_group_perms.type=102 AND user_auth_group_perms.group_id=' . get_request_var('id', 0) . ')';
	    }
	    ...
	    $total_rows = db_fetch_cell("SELECT
			...
			LEFT JOIN user_auth_group_perms
			ON gt.id = user_auth_group_perms.item_id
			AND user_auth_group_perms.type = 102
			AND user_auth_group_perms.group_id = " . get_request_var('id') . "
			$sql_where");
	    ...
	    $sql_query = "SELECT gt.id, gt.name, user_auth_group_perms.group_id
			...
			$nav = html_nav_bar('user_group_admin.php?action=edit&tab=permsrep&id=' . get_request_var('id'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('拓扑图'), 'page', 'main');
			
			form_start('user_group_admin.php?tab=permsrep&id=' . get_request_var('id'), 'chk');
	...
	case 'permsform': // 报表管理 103
	    ...
	    if (get_request_var('associated') == 'true') {
	        ...
	        $sql_where .= ($sql_where != '' ? ' AND ':'WHERE ') . ' (user_auth_group_perms.type=103 AND user_auth_group_perms.group_id=' . get_request_var('id', 0) . ')';
	    }
	    ...
	    $total_rows = db_fetch_cell("SELECT
			...
			LEFT JOIN user_auth_group_perms
			ON gt.id = user_auth_group_perms.item_id
			AND user_auth_group_perms.type = 103
			AND user_auth_group_perms.group_id = " . get_request_var('id') . "
			$sql_where");
	    
	    $sql_query = "SELECT gt.id, gt.name,gt.type, user_auth_group_perms.group_id
			FROM view_plugin_report AS gt
			LEFT JOIN user_auth_group_perms
			ON gt.id = user_auth_group_perms.item_id
			AND user_auth_group_perms.type = 103
			...
			$nav = html_nav_bar('user_group_admin.php?action=edit&tab=permsform&id=' . get_request_var('id'), MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 11, __('拓扑图'), 'page', 'main');
			
			form_start('user_group_admin.php?tab=permsform&id=' . get_request_var('id'), 'chk');
		...
		foreach($perms as $realm) {
			...
			if (isset($user_auth_realms[$realm])) {
			    ...
			    $set = db_fetch_cell_prepared('SELECT realm_id 
                    FROM user_auth_group_realm WHERE group_id = ? AND realm_id = ?', 
...
function user_group() {
	...
	if (cacti_sizeof($group_list)) {
		foreach ($group_list as $group) {
			...
			form_selectable_cell(($group['members'] > 0 ? number_format_i18n($group['members'], 0):'0'), $group['id']);
...
function topo_filter($header_label) {
	...
	function applyFilter() {
		strURL  = 'user_group_admin.php?action=edit&tab=permstopo&id=<?php print get_request_var('id');?>'
		strURL += '&rows=' + $('#rows').val();
		strURL += '&associated=' + $('#associated').is(':checked');
		strURL += '&filter=' + $('#filter').val();
		strURL += '&header=false';
	...
	function clearFilter() {
		strURL = 'user_group_admin.php?action=edit&tab=permstopo&id=<?php print get_request_var('id');?>&clear=true'
		strURL = strURL + '&header=false';
		loadPageNoHeader(strURL);
	}
...
function rep_filter($header_label) {
	...
	function applyFilter() {
		strURL  = 'user_group_admin.php?action=edit&tab=permsrep&id=<?php print get_request_var('id');?>'
		strURL += '&rows=' + $('#rows').val();
		strURL += '&associated=' + $('#associated').is(':checked');
		strURL += '&filter=' + $('#filter').val();
		strURL += '&header=false';
	...
	function clearFilter() {
		strURL = 'user_group_admin.php?action=edit&tab=permsrep&id=<?php print get_request_var('id');?>&clear=true'
		strURL = strURL + '&header=false';
		loadPageNoHeader(strURL);
	}
...
function form_filter($header_label) {
	...
	function applyFilter() {
		strURL  = 'user_group_admin.php?action=edit&tab=permsform&id=<?php print get_request_var('id');?>'
		strURL += '&rows=' + $('#rows').val();
	...
	function clearFilter() {
		strURL = 'user_group_admin.php?action=edit&tab=permsform&id=<?php print get_request_var('id');?>&clear=true'
		strURL = strURL + '&header=false';
		loadPageNoHeader(strURL);
	}
