...
function form_save() {
	...
	cacti_log("根据local_graph_id = $local_graph_id 获取聚合图形id = " .$aggregate_graph_id);
	...
	cacti_log("保存图形后得到graph_templates_graph表id = " .$graph_templates_graph_id);
	...
	cacti_log("根据aggregate_graphs表id=$aggregate_graph_id 更新 title_format = " . $graph_title);
	...
	if (!isset_request_var('template_propogation')) {
		...
		if (!empty($aggregate_graph_id)) {
			...
			cacti_log("查询到之前聚合图形表的的结果 = " . json_encode($old));
			$save_me = 0;
			...
			if ($save_me) {
			    cacti_log("需要保存的数据有改变，更新aggregate_graphs表");
			...
			cacti_log("根据graph_template_id = $graph_template_id 获取 items 信息 = " . json_encode($graph_templates_items));
			...
			cacti_log("根据aggregate_graph_id = $aggregate_graph_id 获取 聚合图形item 信息 = " . json_encode($aggregate_graph_items_old));

			/* update graph template item values with posted values */
			aggregate_validate_graph_items($_POST, $graph_templates_items);
			cacti_log("更新聚合图形item之后的信息 = " .json_encode($graph_templates_items));
...
function form_actions() {
    global $config,$graph_actions, $agg_item_actions,$plugins;
	...
	if (isset_request_var('selected_items')) {
		...
		if ($selected_items != false) {
			...
			}elseif(get_request_var('drp_action') == '10000000'){ // 创建聚合图形阈值
			    if (api_plugin_is_enabled('thold') || in_array('thold', $plugins)) {
    			    ...
    			    if ($selected_items != false) {
    			        ...
    			        if (cacti_sizeof($template)) {
    			            foreach($selected_items as $local_graph_id) {
    			                if (thold_aggregate_create_from_template($local_graph_id, $template, $message)) {
    			                    $created++;
    			                }
    			            }
    			            if (strlen($message)) {
    			                raise_message(10000,'成功创建'.$created.'条阈值<br>' . $message, MESSAGE_LEVEL_INFO);
    			            } else {
    			                raise_message(2000,'没有创建阈值。它们已经存在，或者找不到匹配的组合。', MESSAGE_LEVEL_INFO);
    			            }
    			        } else {
    			            raise_message(0,"阈值没有创建成功由于没有创建阈值！", MESSAGE_LEVEL_ERROR);
    			        }
    			    }
			    }else {
			        raise_message(0,"请先启用阈值插件！", MESSAGE_LEVEL_ERROR);
			    }
			}
		}
	...
	if (isset($graph_array) && cacti_sizeof($graph_array)) {
		if (get_request_var('drp_action') == '1') { // delete
		...
		}else if(get_request_var('drp_action') == '10000000'){ // 聚合创建阈值
		    print "<tr>
				<td class='textArea'>
					<p>您确定要为这些图形创建阈值吗？</p>
					<div class='itemlist'><ul>$graph_list</ul></div>
		    ...
		    $form_array = array(
		        'general_header' => array(
		            'friendly_name' => __("可用的阈值模板"),
		            'method' => 'spacer',
		        ),
		        'thold_template_id' => array(
		            'method' => 'drop_sql',
		            'friendly_name' => __("选择一个阈值模板"),
		            'description' => '',
		            'value' => __('None', 'thold'),
		    ...
		    print '</td></tr></table><table class="cactiTable">';
		    
		    $save_html = "<input type='button' class='ui-button ui-corner-all ui-widget' value='" . __esc('Cancel') . "' onClick='cactiReturnTo()'>&nbsp;<input type='submit' class='ui-button ui-corner-all ui-widget' value='" . __esc('Continue') . "' title='继续'>";
		}
	} else {
		raise_message(40);
		header('Location: aggregate_graphs.php?header=false');
...
function graph_edit() {
	...
	if (!isempty_request_var('id')) {
		...
		$header_label = '[编辑: ' . html_escape(get_graph_title(get_request_var('id'))) . ']';
	}
...
function aggregate_items() {
	...
	$display_text = array(
		...
		'local_graph_id' => array('display' => __('图形ID'), 'align' => 'right', 'sort' => 'ASC'),
	...
	if (cacti_sizeof($graph_list) > 0) {
		foreach ($graph_list as $graph) {
			...
			form_selectable_cell(filter_value($graph['local_graph_id'], '', 'graphs.php?action=graph_edit&id=' . $graph['local_graph_id'],"_blank"), $graph['local_graph_id'], '', 'right');
